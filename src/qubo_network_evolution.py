...
# ============================================================
# 2. PHYSICS ENGINE (Hamiltonian & QUBO)
# ============================================================
@dataclass
...
def hill_climbing_lattice(lattice: QuantumLattice2D, simulator: AerSimulator, h_terms: List[HamiltonianTerm], generations: int = 100, mutation_rate: float = 0.3):
    print("\n--- STARTING QUBO LATTICE EVOLUTION ---")
    
    best_energy = evaluate_lattice_energy(lattice, simulator, h_terms)
    print(f"Gen 000 [START] | Global Energy: {best_energy:.4f}")
...
# ============================================================
# 5. MAIN EXECUTION (QUBO IMPLEMENTATION)
# ============================================================
if __name__ == "__main__":
    sim = AerSimulator()
    
    # 1. Create a 2x2 grid (4 LEDs total)
    my_city_grid = QuantumLattice2D(rows=2, cols=2)
    
    # 2. Define the QUBO Hamiltonian (Nodes and Edges)
    # Grid Layout:
    # Q0 --- Q1
    # |      |
    # Q2 --- Q3
    ...
    terms = [
        # LINEAR TERMS: Reward for turning LEDs on (Z_i)
        HamiltonianTerm(1.0, "Z", (0,)),
        HamiltonianTerm(1.0, "Z", (1,)),
        HamiltonianTerm(1.0, "Z", (2,)),
        HamiltonianTerm(1.0, "Z", (3,)),
        
        # QUADRATIC TERMS (QUBO): Penalty for adjacent LEDs turned on together (Z_i * Z_j)
        HamiltonianTerm(2.0, "ZZ", (0, 1)), # Row 0 neighbor penalty
        HamiltonianTerm(2.0, "ZZ", (2, 3)), # Row 1 neighbor penalty
        HamiltonianTerm(2.0, "ZZ", (0, 2)), # Col 0 neighbor penalty
        HamiltonianTerm(2.0, "ZZ", (1, 3))  # Col 1 neighbor penalty
    ]
    
    # 3. Run Evolution
    # Because of the conflicting rules (turn on vs. stay apart), we use 300 generations to give it time to learn.
    hill_climbing_lattice(my_city_grid, sim, terms, generations=300, mutation_rate=0.4)