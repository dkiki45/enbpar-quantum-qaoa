...
# ============================================================
# 3. SELF-TEST (sanity check for the derivation above)
# ============================================================
if __name__ == "__main__":
    # Toy problem: 3 variables, 1 edge (0,1)
    linear, quadratic, offset0 = build_qubo(n_vars=3, edges=[(0, 1)], alpha=1.0, beta=2.0)
    print("QUBO linear:", linear)
    print("QUBO quadratic:", quadratic)

    terms, offset = qubo_to_ising(linear, quadratic, offset0)
    print("\nIsing terms:")
    for t in terms:
        print(f"  {t.coefficient:+.3f} * {t.pauli}{t.qubits}")
    print(f"Offset (global constant): {offset:+.3f}")

    # Brute-force verification by enumeration (3 variables -> 8 states)
    def classical_cost(x, linear, quadratic, offset0):
        c = offset0
    ...
    def ising_energy(x, terms, offset):
        z = {i: (1 - 2 * xi) for i, xi in enumerate(x)}  # x=0 -> Z=+1, x=1 -> Z=-1
        e = offset
        for t in terms:
            if t.pauli == "Z":
                e += t.coefficient * z[t.qubits[0]]
            elif t.pauli == "ZZ":
                e += t.coefficient * z[t.qubits[0]] * z[t.qubits[1]]
        return e

    print("\nClassical QUBO vs reconstructed Ising energy comparison (should match):")
    import itertools
    for x in itertools.product([0, 1], repeat=3):
        c = classical_cost(x, linear, quadratic, offset0)
        e = ising_energy(x, terms, offset)
        ok = "OK" if abs(c - e) < 1e-9 else "MISMATCH"
        print(f"  x={x}  C(x)={c:+.3f}  E_ising={e:+.3f}  [{ok}]")